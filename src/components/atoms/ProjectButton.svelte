...
{:else if type === 'wonky'}
	<a
		href={url}
		class="text-black relative block cursor-pointer border-none"
		role="button"
		onclick={handleClick}
	>
		{#if !active}
			...
			{#each words as word, index}
				...
				{:else}
					<span
						class={index === 1
							? 'mr-3 block rotate-6 text-xl font-medium'
							: 'text-xl font-medium'}>{word}</span
					>
				{/if}
			{/each}
		{/if}
...
{:else}
	<a href={url} onclick={handleClick} class="{baseClasses} border-none" role="button">
		{label}
	</a>
{/if}
