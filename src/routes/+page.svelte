<script lang="ts">
    ...
    import type { PersonResponse, ActionData }  from '$lib//types';

    export let data;
    export let form: ActionData;
    console.log(form);

    const { pageTitle, metadescription, url, temp, image, squareImage } = data as PageData;
    ...
    function handleFormSubmit(event: SubmitEvent) {
        ...
        .then((data: { success: boolean; error?: { message: string }; data?: PersonResponse }) => {
            isSubmitting = false;
            if (!data.success) {
...
<div class="grid grid-rows-2 h-screen w-screen">
    <div class="bg-extremeCaution justify-center h-full flex content-center">
        <h1 class="text-extremeDanger self-center font-anton fluid-text-xl uppercase text-center">
            Cooler Together
        </h1>
    </div>
    <div class="bg-danger justify-center h-full flex content-center">
        ...
        {:else if form?.success}
            <p>Success! User {form.data?.given_name} {form.data?.family_name} with email {form.data?.email} was added successfully.</p>
        {:else if form?.error || errorMessage}
            <p>Submission failed: {form?.error?.message || errorMessage}</p>
        {:else}
            <Signup action="/" on:submit={handleFormSubmit}></Signup>
        {/if}
    </div>
</div>

<div class="fixed top-24 left-24">
