<script lang="ts">
	...
	type EditorBlock =
		...
		| (AcfHeaderAnimationType & NormalizedBlock)
	...
	const classNames = (align: string) => {
		...
		switch (align) {
			...
			case 'wide':
				...
				break
			case 'none':
				baseClasses = 'w-full lg:max-w-screen-md mx-auto '
				break
			case 'center':
		...
		// Add className from attributes if it exists
		const customClassName = block.attributes?.className || '';

		return `${baseClasses} ${spacingClasses} ${customClassName}`.trim(); // Combine base classes with spacing and custom classes
	}

	const extraClassName:string = block.attributes?.className ?? ''
	...
	const getBgClass = (bgColor: string | null): string => {
		switch (bgColor) {
			case 'caution':
				return 'bg-caution'
			case 'extremecaution':
				return 'bg-extremecaution'
			case 'danger':
				return 'bg-danger'
			case 'extremedanger':
				return 'bg-extremedanger'
			case 'black':
				return 'bg-black'
			case 'white':
				return 'bg-white'
...
<div
	class="{extraClassName} {block.name.toLowerCase().replace('/', '-')} {verticalAlignmentClasses(verticalAlignment)} {classNames(align)} {getBgClass(
		bgColor
	)} !px-0"
>
